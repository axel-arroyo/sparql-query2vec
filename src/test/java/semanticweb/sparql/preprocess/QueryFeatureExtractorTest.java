    ...
    public void testpProcessTPFJoinsV1() {
        ...
        for (int i = 0; i < queries.size(); i++) {
            ...
            try {
                ...
                switch (i) {
                    case 0:
                        Assert.assertEquals  (aList.get(0).get(2), String.valueOf(QueryFeatureExtractor.ONE_WAY_TWO_PREDS));
                        break;
                    case 1:
                        Assert.assertEquals (aList.get(0).get(2), String.valueOf(QueryFeatureExtractor.TWO_OUTGOING_PRED_VAR_URI));
                        break;
                    case 2:
                        Assert.assertEquals (aList.size(),0);
                        break;
                    case 3:
                        Assert.assertEquals (aList.get(0).get(2), String.valueOf(QueryFeatureExtractor.TWO_INCOMING_PREDICATES));
                        break;
                }
            } catch (Exception ex) {
                ex.printStackTrace();
