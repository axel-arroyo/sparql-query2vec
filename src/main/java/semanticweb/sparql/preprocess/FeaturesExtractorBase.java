...
public abstract class FeaturesExtractorBase {
     ...
     * @param queryColumn Csv column that contain query string( Csv must contain
     *                    other data)
     ...
     */
    public ArrayList<ArrayList<String>> getArrayQueriesMetaFromCsv(String url, boolean header, String delimiter,
            int queryColumn, int idColumn, int execTimeColumn, int length) {
        return this.getArrayQueriesMetaFromCsv(url, header, delimiter, queryColumn, idColumn, execTimeColumn, -1,
                length);
    }
     ...
     * @param idColumn    Csv column that contain the query id
     * @param length      Length of the queries with cardinality to load > zero
     * @return a list of Queries in format
     *         [idQuery,Query,execTimeColumn,Cardinality]
     */
    public ArrayList<ArrayList<String>> getArrayQueriesMetaFromCsv(String url, boolean header, String delimiter,
            int queryColumn, int idColumn, int execTimeColumn, int cardinalityColumn, int length) {
        String row;
        ...
        try {
            ...
            if (header) {
                ...
                csvColumns = headerCols.split(delimiter).length;
            }
