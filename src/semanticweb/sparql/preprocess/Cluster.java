...
//		xtestFeaturePs.close();
	...
	public void loadDistanceMatrix() throws IOException {
		...
		distanceMatrix = new float[9984][9984];
		int count = 0;
		while(in.hasNext()) {
			String[] row = in.nextLine().split(",");
			int i = count;
			for (int j = 0; j < row.length; j++) {
				float d = Float.parseFloat(row[j]);
...
//		Scanner in = new Scanner(new FileInputStream("/home/daniel/Documentos/ML/rhassan/graph-edit-distance/data/config/100/salva/hungar.csv"));
//		int count = 0;
//		float[][] istanceMatrix = new float[3][3];
//		while(in.hasNext()) {
//			String[] row = in.nextLine().split(",");
//			int i = count;
//			for (int j = 0; j < row.length; j++) {
//				float d = Float.parseFloat(row[j]);
//				istanceMatrix[i][j] = istanceMatrix[j][i] = d;
//			}
//
////			if(count%10000000==0) {
